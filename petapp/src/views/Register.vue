<template>
  <div class="login">
   ...
   <van-cell-group class="inputstyle">
    ...
    <van-field class="unamestyle" v-model="uname" label="用户名:" clearable placeholder="请输入手机号" :error-message="errorMessage.uname"></van-field>
    <van-field class="upwdstyle" v-model="upwd" label="密码:" clearable type="password" placeholder="请输入密码"></van-field>
    <van-field class="uupwdstyle" v-model="uupwd" label="确认密码:" clearable type="password" placeholder="请再次输入密码"></van-field>
     <!-- 登录按钮 -->
     <van-button class="loginstyle" @click="login">注册</van-button>
  ...
  methods:{
    login(){
      ...
      if(nreg.test(n)==false){
        //验证不成功 弹出层弹出提示信息
        this.errorMessage.uname="手机格式有误,请重新输入";
        // this.show=true;
        return;
      }
